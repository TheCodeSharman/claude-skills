...
A related move: when extracting, give the helper the *narrow* responsibility and let the caller keep its own convention. In the example above, `parseHeaderLine` returns a faithful `{ name, value }`; the lowercasing belongs to the collection's `parse`/`get` path because lookup also lowercases. The owner keeps the canonicalization; the helper just parses.
...
- **"Extra object allocation per iteration."** Same — allocations are essentially free at the application layer. Real performance concerns almost always live in IO, network latency, rendering, and bulk data work — orders of magnitude above anything function-shape-related.

If you find yourself reaching for any of these as a reason not to extract, stop. The actual question is whether the name carries intent. Perf objections are only legitimate in genuine hot paths (per-frame rendering, pixel work, thousands-of-records processing), and even there the first intervention is "do less work", not "inline the helper".
...
Reserve comments for the non-obvious *why* — hidden constraints, workarounds, invariants. See [write-docs](../write-docs/SKILL.md) for the source-comment-vs-doc seam.
...
Tidies in *adjacent* untouched code: open a separate branch. The **one-change = one-responsibility** rule still applies — drive-by tidies on unrelated files turn a focused change into a sprawling one. Splitting up front costs minutes; splitting after the fact costs an hour of rebasing and re-running checks.
...
For broad sweeps — touching many files, renaming an interface, reshaping a module — flag the intent to the user before doing the work, even if every individual move is a tidy. Code happens fast in these sessions; the human reviewer is the project's refactor-detector, and a surfaced proposal gives them a checkpoint to decide refactor-now vs. carry-on.
...
- [write-docs](../write-docs/SKILL.md) — when a source comment is justified vs. when the content belongs in a doc.
- [open-pr](../open-pr/SKILL.md) — one-change-one-responsibility, and how drive-by tidies split off.
